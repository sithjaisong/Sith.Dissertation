...
#V(cnet[[i]])$frame.color <- "white" #adjustcolor("slateblue1", alpha.f = .6)
...
Ecol <- c("#D94801", "#F16913" , "#FD8D3C", "gray70", "gray70", "#4292C6", "#2171B5" , "#084594")

Ecolc <- cut(results_sub.by.country[[i]]$rho, breaks = c(-1, -0.75, -0.50, -0.25, 0, 0.25, 0.50, 0.75,  1), 
            include.lowest = TRUE)

E(cnet[[i]])$color <- Ecol[Ecolc]

Vcol <- c("#6BAED6", "#4292C6", "#2171B5", "#08519C")

Vcolc <- cut(degree(cnet[[i]])/max(degree(cnet[[i]])), breaks = 4, include.lowest = TRUE)

V(cnet[[i]])$color <- Vcol[Vcolc] #adjustcolor("slateblue1", alpha.f = .6)
V(cnet[[i]])$frame.color <- Vcol[Vcolc]
# == plot network model
